...
int main(void)
{
	...
	task &= !p3[0][3] & !p4[8][4];
	for (unsigned i = 0; i < N - 1; i++) {
		task &= !(p4[i][4] ^ p2[i + 1][3]);
	}

	//ограничение типа 4 n4=4
	...
	//И пусть Нейтен будет англом
	for (unsigned i = 0; i < N; i++) task &= !(p2[i][3] ^ p1[i][2]);

	task &= !(p1[6][5] ^ p3[8][7]);
	task &= !(p1[6][8] ^ p3[6][8]);

	restrict4(task,p2,6,p3,7);
